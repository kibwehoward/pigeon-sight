...
Package and distribute analysis results to end users in a form they can act on. This is the final stage of the pipeline. The reference implementation uses **GeoServer** for web services and **QGIS** for static map products.
...
- Delivery specification:
  ...
  - Delivery deadline and method (GeoServer service, shared drive, email, portal)
- Branding or formatting requirements

## Outputs

- Delivered artifacts in the stakeholder-specified format, which may include:
  - Interactive web map (GeoServer WMS/WFS, served via QGIS Server or a web client)
  - Static map products (QGIS print layout exported as PDF or PNG)
  - Data exports (GeoPackage, GeoJSON, Shapefile, CSV, KML)
  - Written report with embedded maps and analysis results
  - GeoServer layer or service endpoint with access instructions
...
- GeoServer services return correct data and render without errors
...
| GeoServer service misconfigured or returns errors | Review layer configuration and data store connection; retest before notifying recipients |
...
3. Record the complete lineage: capture → WebODM processing → validation → cleaning → PostGIS ingestion → analysis → delivery
